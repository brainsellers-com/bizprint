...
- **Agent ツールのサブエージェントはバックグラウンドで動作してよい。** 対話セッションでは fork mode が既定で有効なためサブエージェントは原則バックグラウンドで起動され、フォアグラウンドは指定できない。権限を要するツールの承認プロンプトはメインセッションに表示される（公式: https://code.claude.com/docs/en/sub-agents#run-subagents-in-foreground-or-background）。ただし `AskUserQuestion` はフォアグラウンド・バックグラウンドを問わずサブエージェント（fork を除く）では使えないため、ユーザー確認を含むスキル（`/start-task` 等）はサブエージェントに委任せずメインセッションで実行する。
